...
class Special_course extends My_Controller {
	...
	public function index($page = 1) {
		...
		// 分页
		$config ['base_url'] = site_url ( 'course/special_course/index' );
		...
		$this->template->display ( 'course/special_course/list.html', $data );
	}
	...
	public function detail($course_id = '') {
		...
		if ($course_id) {
			...
			$sql = "SELECT * FROM w_course WHERE course_id = {$course_id} AND course_type = '3'";
		...
		$this->template->display ( 'course/special_course/detail.html', $data );
	}
...
// 		$data['summary'] =  $this->input->post ( 'summary' );
		...
		redirect ( base_url () . 'course/special_course' );
	}
